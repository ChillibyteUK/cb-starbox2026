...
# Usage:
#   ./populate_acf_from_block.sh blocks/cb-example.php
...
if [ "$#" -lt 1 ]; then
  ...
  echo "Example: $0 blocks/cb-cta.php" >&2
  exit 1
fi
...
def labelize_field_name(name: str) -> str:
    # Remove leading cb_ / cb- prefix from labels (keep field names unchanged)
...
# Normalize existing labels so cb_ prefix is not shown in ACF UI labels.
